...
def _load_f_coref_model(args):
    ...
    # Freeze certain weights of pretrained model
    if args.freeze_transformer_layers:
        transformer_prefix = model.base_model_prefix
        for named_param in model.named_parameters():
            if transformer_prefix in named_param[0]:
                named_param[1].requires_grad = False

    if args.freeze_custom_layers:
        for named_param in model.named_parameters():
            for layer_str in args.freeze_custom_layers:
                if layer_str in named_param[0]:
                    named_param[1].requires_grad = False
